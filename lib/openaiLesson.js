...
export async function buildLessonWithAI({
  ...
  // If user requested a specific slide count, use it
  if (requestedSlideCount && requestedSlideCount >= 4 && requestedSlideCount <= 20) {
    slideCount = String(requestedSlideCount);
    if (requestedSlideCount <= 6) {
...
- Verner's Law/Theory → Linguistics/Historical Linguistics (NOT Chemistry)
...
For Chemistry topics, follow this strict format for bullet points:

A. Concept Overview (2-3 bullets)
   - What this topic is about in simple terms

B. Key Chemical Idea
   - One clear idea students must understand

C. Process/Reaction Explanation (Step-wise)
   - 3-5 short steps explaining what happens
   - Each step should align with visual changes in the diagram

D. Exam-Oriented Keywords
   - 5-7 important terms (no sentences)

E. Common Student Mistakes
   - 2-3 typical misconceptions

F. Teacher Prompt (Optional)
   - 1 question the teacher can ask while teaching

CHEMISTRY STYLE RULES:
- Do NOT write long paragraphs
- Use simple, teacher-friendly bullet points
- Assume a diagram (SVG) will explain structure/process
- Text must SUPPORT the diagram, not repeat it
- Avoid unnecessary theory or advanced terminology
- Use present tense
- Avoid formulas unless essential
- Avoid real-life examples unless specifically needed
- Do not include answers to numerical problems
` : ""}
...
1. ALL content must be specifically about "${topic}" in the context of ${subject}
2. Every diagram, example, and question must relate directly to "${topic}"
3. Do NOT use generic placeholders or content from other topics
4. Every slide must have a "content" object with appropriate fields
5. For concept slides: bullets must explain "${topic}" specifically
6. For question slides: all questions must test understanding of "${topic}"
