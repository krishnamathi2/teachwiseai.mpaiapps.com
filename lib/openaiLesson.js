...
import { requestModelContent } from "./modelProvider";
...
export async function buildLessonWithAI({
  ...
  if (requestedSlideCount && requestedSlideCount >= 2 && requestedSlideCount <= 20) {
    ...
    if (requestedSlideCount <= 3) {
      ...
      paceGuidance = "Keep content extremely brief and essential. Only cover the absolute core concepts. Maximum 2-3 bullet points per slide. For 2-3 slides, use ONLY 'concept' type slides - do NOT create summary, questions, or worked_example slides. Each slide should cover one key aspect of the topic.";
...
IMPORTANT: Your response MUST be valid, complete JSON. If you cannot fit all ${slideCount} slides, return as many as possible but ensure the JSON is valid and not truncated. Do NOT include any extra text, explanations, or markdown—just the JSON object.

Each slide must be CONCISE: Limit to 2-3 bullet points per slide, keep explanations brief, and avoid lengthy text. Use short, clear sentences. Do NOT repeat information across slides.
...
1. Generate up to ${slideCount} slides (not more than ${slideCount}). If you cannot fit all, return fewer, but the JSON must be valid and complete.
...
Start your response with the opening brace { and ensure the JSON is never truncated.`;

  let response;

  // Perplexity batch limit (max reliable slides per call)
  const BATCH_LIMIT = 4;
  let allSlides = [];
  let meta = null;
  let totalRequested = parseInt(slideCount, 10) || 2;
  let batchStart = 0;
  let batchNum = 1;
  while (batchStart < totalRequested) {
    let batchSize = Math.min(BATCH_LIMIT, totalRequested - batchStart);
    let batchPrompt = userPrompt + `\\n\\nONLY generate slides ${batchStart + 1} to ${batchStart + batchSize} (numbered in order).`;
    let lessonJson;
    let currentSlideCount = batchSize;
    let lastError = null;
    let batchSlides = [];
    while (currentSlideCount >= 2) {
      const retryPrompt = batchPrompt.replace(/up to \\d+ slides|exactly \\d+ slides|not more than \\d+|${slideCount}/gi, `up to ${currentSlideCount} slides`);
      try {
        const prompt = `${systemPrompt}\n\n${retryPrompt}`;
        const raw = await requestModelContent(prompt);
        if (typeof raw === "string") {
          console.error("[buildLessonWithAI] Raw Perplexity response:", raw);
          ...
          // Attempt to extract JSON object from within extra text
          ...
          try {
            lessonJson = JSON.parse(jsonText);
            break;
          } catch (jsonErr) {
            // Try to salvage partial slides array from malformed JSON
            try {
              // Find the slides array
              const slidesStart = jsonText.indexOf('"slides"');
              if (slidesStart !== -1) {
                const arrStart = jsonText.indexOf('[', slidesStart);
                if (arrStart !== -1) {
                  // Try to find the matching closing bracket for the array
                  let arrEnd = jsonText.lastIndexOf(']');
                  if (arrEnd === -1 || arrEnd < arrStart) {
                    // Try to close the array at the last valid object
                    arrEnd = jsonText.lastIndexOf('}');
                    if (arrEnd !== -1 && arrEnd > arrStart) {
                      // Try to extract up to last valid object
                      let arrText = jsonText.substring(arrStart, arrEnd + 1);
                      // Try to fix trailing commas
                      arrText = arrText.replace(/,\s*\]$/, ']');
                      try {
                        const slidesArr = JSON.parse(arrText);
                        if (Array.isArray(slidesArr) && slidesArr.length > 0) {
                          batchSlides = batchSlides.concat(slidesArr);
                          console.warn('[buildLessonWithAI] Salvaged partial slides from malformed JSON.');
                        }
                      } catch (arrErr) {
                        // Could not salvage
                      }
            ...
            lastError = jsonErr;
            console.error("[buildLessonWithAI] Invalid JSON from Perplexity after cleaning:", jsonErr.message);
          }
        } else {
          lessonJson = raw;
          break;
      ...
      currentSlideCount = Math.floor(currentSlideCount / 2);
      if (currentSlideCount < 2) {
        break;
      }
      console.warn(`[buildLessonWithAI] Retrying with slide count: ${currentSlideCount}`);
    }
    // Prefer full lessonJson if available, else use salvaged slides
    if (lessonJson && lessonJson.slides && Array.isArray(lessonJson.slides)) {
      if (!meta && lessonJson.meta) meta = lessonJson.meta;
      allSlides = allSlides.concat(lessonJson.slides);
    } else if (batchSlides.length > 0) {
      allSlides = allSlides.concat(batchSlides);
    } else {
      console.warn('[buildLessonWithAI] No slides could be salvaged from this batch.');
    }
    batchStart += batchSize;
    batchNum++;
  }
  if (allSlides.length === 0) {
    throw new Error("Perplexity API returned invalid JSON, and no slides could be salvaged from any batch. Please try again or check the prompt format. Last error: " + (lastError?.message || lastError));
  }
  let lesson = { meta: meta || {}, slides: allSlides };
  ...
  // Don't filter out any slides - we want to keep the exact count requested by the user

  if (lesson?.slides) {

    // Recursively flatten bullets: arrays → multiple bullets, objects → key-value bullets, strings as-is
    const flattenBullets = (b) => {
      if (typeof b === 'string') return [b];
      if (Array.isArray(b)) return b.flatMap(flattenBullets);
      if (typeof b === 'object' && b !== null) {
        return Object.entries(b).map(([k, v]) => Array.isArray(v)
          ? `${k}: ${v.join('; ')}`
          : `${k}: ${v}`
        );
      }
    ...
    lesson.slides = lesson.slides.map((slide, index) => {
      ...
      if (needsBullets && (!slide.content.bullets || slide.content.bullets.length === 0)) {
        console.warn(`[buildLessonWithAI] Slide ${index + 1} \"${slide.title}\" missing bullets, adding fallback`);
      ...
      // Recursively flatten bullets for PDF rendering
      if (needsBullets && slide.content.bullets) {
        slide.content.bullets = slide.content.bullets.flatMap(flattenBullets);
        console.log(`[buildLessonWithAI] Slide ${index + 1} has ${slide.content.bullets.length} bullets`);
      }
  ...
  // Log final slide count vs requested
  const finalSlideCount = lesson.slides?.length || 0;
  const requestedOrDefault = requestedSlideCount || 'default';
  console.log(`[buildLessonWithAI] Final result: ${requestedOrDefault} (default) slides generated (requested: ${requestedOrDefault})`);

  if (lesson.slides && lesson.slides[0]) {
    console.log("[openaiLesson] First slide sample:", JSON.stringify(lesson.slides[0], null, 2));
  }
  if (requestedSlideCount && finalSlideCount !== requestedSlideCount) {
    console.warn(`[buildLessonWithAI] WARNING: Generated ${finalSlideCount} slides but user requested ${requestedSlideCount}`);
  }
  return lesson;
}